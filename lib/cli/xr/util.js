...
// Adds the options every device-bound xr subcommand shares. A blanket
// yargs .env() prefix cannot be used here: the CLI runs in strict mode,
// so an exported STF_XR_* variable for one subcommand would make every
// other subcommand fail with "Unknown argument". Instead the shared
// options read their environment fallbacks explicitly.
module.exports.commonOptions = function(yargs) {
  return yargs
    .option('serial', {
      alias: 's'
    , describe: 'The serial number of the target device ($STF_XR_SERIAL).'
    , type: 'string'
    , demandOption: true
    , default: process.env.STF_XR_SERIAL
    })
    .option('adb', {
      describe: 'Path to the ADB binary (defaults to $STF_XR_ADB, $ADB or `adb`).'
    , type: 'string'
    , default: process.env.STF_XR_ADB
    })
    .option('timeout', {
      describe: 'Timeout in milliseconds for device operations ($STF_XR_TIMEOUT).'
    , type: 'number'
    , default: process.env.STF_XR_TIMEOUT ?
        parseInt(process.env.STF_XR_TIMEOUT, 10) : 30000
    })
    .option('json', {
